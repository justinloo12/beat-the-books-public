...
class SimulationModelService:
    ...
    def _plate_appearance_distribution(
        ...
        matchup_score = float(matchup.get("matchup_score") or 50.0)
        pitcher_xba = float(opposing_pitcher.get("xba") or 0.245)
        pitcher_barrel = float(opposing_pitcher.get("barrel_pct") or 0.08)
        pitcher_hard_hit = float(opposing_pitcher.get("hard_hit_pct") or 0.36)
        pitcher_k = float(opposing_pitcher.get("weighted_k_pct") or 0.22)
        pitcher_bb = float(opposing_pitcher.get("weighted_bb_pct") or 0.08)
        weather_multiplier = float(((matchup.get("weather") or {}).get("weather_multiplier")) or 1.0)
